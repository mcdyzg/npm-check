...
function interactive(currentState) {
    ...
    return new Promise(resolve => inquirer.prompt(questions, resolve)).then(
        answers => {
            ...
            if (!currentState.get('global')) {
                if (saveDependencies.length) {
                    // 1. if use yarn, delete --save
                    if (currentState.get('installer') === 'yarn') {
                ...
                if (saveDevDependencies.length) {
                    // 1. if use yarn, add -D instead
                    if (currentState.get('installer') === 'yarn') {
                        saveDevDependencies.unshift('-D')
                    } else {
